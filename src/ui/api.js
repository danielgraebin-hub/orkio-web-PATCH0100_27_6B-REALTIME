...
export async function chat({ thread_id = null, agent_id = null, message, top_k = 6, token, org, trace_id = null } = {}) {
  const tenant = (org ?? getTenant() ?? "public").trim() || "public";
  return apiFetch("/api/chat", {
    method: "POST",
    token,
    org: tenant,
    body: { tenant, thread_id, agent_id, message, top_k, trace_id },
  });
}
...
export async function chatStream({ thread_id = null, agent_id = null, message, top_k = 6, token, org, trace_id = null } = {}) {
  const tenant = (org ?? getTenant() ?? "public").trim() || "public";
  ...
  const body = JSON.stringify({ tenant, thread_id, agent_id, message, top_k, trace_id });
